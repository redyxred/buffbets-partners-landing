	...
	return (
		<div className="mb-4">
			<button
				onClick={onToggle}
				className={`
                    w-full flex items-center justify-between px-6 py-4 rounded-full
                    transition-all duration-300 text-left
                    border border-transparent cursor-pointer
                    ${isOpen ? "bg-[#1A1A1A] border-white/5" : "bg-background-lighter hover:bg-[#120D26]"}
                `}
			>
				<span className="text-white font-medium text-lg md:text-xl pr-4 pl-2">
					{question}
				</span>
			...
			<AnimatePresence initial={false}>
				{isOpen && (
					...
					>
						<div className="px-6 pt-2 pb-2 text-gray-400 leading-relaxed text-base md:text-lg">
							{answer}
						</div>
					</motion.div>
				)}
